...
export default function ResponsePanel({
  ...
  return (
    <div
      className={`rounded-xl border flex flex-col flex-1 min-h-[220px] overflow-hidden ${clsCard}`}
    >
      <div
        ...
        {currentTab.response && (
          <div className="flex gap-4 text-[11px] font-mono">
            <div>
              ...
              <span
                className={
                  currentTab.response.status >= 200 &&
                  currentTab.response.status < 300
      ...
      <div
        className={`flex-1 p-3 overflow-auto font-mono text-xs ${clsTerminal}`}
      >
        {currentTab.loading && (
        ...
        {!currentTab.loading && !currentTab.response && (
          <div className="h-full flex items-center justify-center text-zinc-500 italic text-xs">
            Enter a valid URL and click Send to see the actual server response.
          </div>
        )}
        {!currentTab.loading && currentTab.response && (
          <pre
