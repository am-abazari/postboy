    ...
    return (
        <div className="flex flex-col gap-1.5">
            <div className={`flex gap-2 p-1.5 rounded-xl border ${clsCard}`}>
                <select value={currentTab.method} onChange={e => updateCurrentTab({method: e.target.value})}
                ...
                <input type="text" value={currentTab.url} onChange={e => updateCurrentTab({url: e.target.value})}
                ...
                <button onClick={handleClearAll}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${isDark ? 'bg-zinc-800 hover:bg-zinc-700 text-zinc-300' : 'bg-slate-100 hover:bg-slate-200 text-slate-600'}`}>Reset
                </button>
                <button onClick={handleSend} disabled={currentTab.loading}
                    ...
                    {currentTab.loading ? <span
                        className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin"/> : 'Send'}
                </button>
            </div>
            {currentTab.validationError &&
                <p className="text-[11px] text-red-500 font-medium px-2">⚠️ {currentTab.validationError}</p>}
        </div>
    );
}