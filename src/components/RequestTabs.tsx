        ...
        <div
            ...
            {tabs.map(t => (
                <div key={t.id}
                    ...
                    <span onClick={() => setActiveTabId(t.id)} className="truncate max-w-28">{t.name}</span>
                    {tabs.length > 1 && (
                        <button onClick={(e) => {
                            e.stopPropagation();
                        ...
                        }} className="hover:text-red-500 text-[10px] ml-1">✕</button>
                    )}
                </div>
            ))}
            <button onClick={handleAddNewTabFromSection}
