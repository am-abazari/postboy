...
export default function Sidebar({
  ...
  return (
    ...
    >
      ...
      >
        <span className={`text-xs font-semibold block mb-1 ${clsMuted}`}>
          مدیریت مجموعه‌ها (JSON)
        </span>
        <div className="flex gap-2">
          <button
            onClick={exportCollections}
            className={`flex-1 py-1.5 px-2 rounded text-xs transition ${isDark ? "bg-zinc-800 hover:bg-zinc-700 text-zinc-300" : "bg-slate-100 hover:bg-slate-200 text-slate-700"}`}
          >
            خروجی (Export)
          </button>
          <label
      ...
      <div className="flex flex-col gap-2 flex-initial min-h-0 overflow-hidden">
        <span className={`text-xs font-semibold ${clsMuted}`}>
                ...
                <div className="flex justify-between items-center mb-1 group">
                  {isEditingThisCol ? (
                    <div className="flex items-center gap-1 flex-1">
                      ...
                      />
                      <button
                        onClick={(e) => handleSaveCollectionName(col.id, e)}
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity ml-1 shrink-0">
                        <button
                          onClick={(e) =>
                            handleStartRenameCollection(col.id, col.name, e)
                          }
                          className="text-[10px] hover:text-orange-500"
                        >
                          ✏️
                        </button>
                        <button
                          ...
                          {isEditingThisReq ? (
                            <div className="flex items-center gap-1 flex-1">
                              <input
                                ...
                                onChange={(e) =>
                                  setEditingReqName(e.target.value)
                                }
                              ...
                              <button
                                onClick={(e) =>
                                  handleSaveRequestName(col.id, req.id, e)
                                }
                            ...
                            <>
                              <button
                                onClick={() => loadSavedRequest(req, col.id)}
                                className={`text-left text-[11px] hover:text-orange-500 truncate flex-1 block ${isDark ? "text-zinc-400" : "text-slate-600"}`}
                              >
                                <span className="text-orange-500 font-mono text-[9px] mr-1 inline-block w-8 uppercase">
                                  {req.method}
                                </span>{" "}
                                {req.name}
                              </button>
                              <div className="flex items-center gap-1 opacity-0 group-hover/req:opacity-100 transition-opacity ml-1 shrink-0">
                                <button
                                  onClick={(e) =>
                                    handleStartRenameRequest(
                                      req.id,
                                  ...
                                  className="text-[9px] hover:text-orange-500"
                                >
                                  ✏️
                                </button>
                                <button
                                  onClick={(e) =>
                                    handleDeleteRequest(col.id, req.id, e)
                                  }
                                  className="text-[9px] hover:text-red-500"
                                >
                                  🗑️
                                </button>
                              </div>
      ...
      <div className="flex flex-col gap-1.5 flex-1 min-h-[160px] border-t pt-3 border-dashed border-zinc-850 overflow-hidden">
        <span
          className={`text-[11px] font-bold tracking-wide uppercase ${clsMuted}`}
        >
          تاریخچه (History)
        </span>
        <div
        ...
        >
          {history.map((hist) => (
            ...
            >
              <div className="flex justify-between items-center w-full">
                <span
                  className={`text-[9px] font-mono font-bold ${hist.method === "GET" ? "text-emerald-500" : "text-orange-500"}`}
                >
                  {hist.method}
                </span>
              ...
              <span
                className={`text-[10px] font-mono truncate w-full block text-right ${isDark ? "text-zinc-400 group-hover:text-zinc-200" : "text-slate-600 group-hover:text-slate-900"}`}
              >
                {hist.url}
              </span>
            </button>
          ...
          {history.length === 0 && (
            <div className="text-[10px] text-zinc-500 italic text-center py-4 my-auto">
              تاریخچه‌ای نیست
            </div>
          )}
        </div>
