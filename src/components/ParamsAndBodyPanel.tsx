...
interface ParamsAndBodyPanelProps {
    ...
    updateRow: (type: 'params' | 'headers', index: number, field: 'key' | 'value', value: string) => void;
    ...
    return (
        <div className={`rounded-xl border flex flex-col h-54 overflow-hidden ${clsCard}`}>
            <div
                ...
                <button onClick={() => setSubTab('headers')}
            ...
            <div className="flex-1 p-3 overflow-y-auto">
                        ...
                        {currentTab.headers.map((header, idx) => (
                            <div key={idx} className="flex gap-2 items-center">
                                ...
                                <input type="text" placeholder="Value" value={header.value}
                                       onChange={e => updateRow('headers', idx, 'value', e.target.value)}
                                       className={`border rounded px-2.5 py-1 text-xs flex-1 focus:outline-none font-mono ${clsInput}`}/>
                                <button onClick={() => removeRow('headers', idx)}
                                        className="text-zinc-400 hover:text-red-500 p-1 text-xs transition">✕
                                </button>
                            </div>
                        ))}
                        <button onClick={() => addRow('headers')}
                                className="text-[11px] text-orange-600 hover:text-orange-500 mt-1.5 self-start font-semibold">+
                            Add Header Row
                        </button>
                    </div>
                ...
                {subTab === 'json' && (
                    <textarea value={currentTab.bodyJson} onChange={e => updateCurrentTab({bodyJson: e.target.value})}
                ...
                {subTab === 'raw' && (
                    <textarea value={currentTab.bodyRaw} onChange={e => updateCurrentTab({bodyRaw: e.target.value})}
